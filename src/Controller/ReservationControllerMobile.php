...
class ReservationControllerMobile extends AbstractController
{
    ...
    #[Route("/ReservationJson/{numRes}", name: "reservation")]
    public function ReservationId($numRes, NormalizerInterface $normalizer, ReservationRepository $repo)
    {
        $reservation = $repo->find($numRes);
        $reservationNormalises = $normalizer->normalize($reservation, 'json', ['reservations' => "reservations"]);
